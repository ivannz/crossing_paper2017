...
class WeierstrassFunction(BaseGenerator):
    r"""A class to produce sample paths of a random Weierstrass function
    over :math:`[0,1]` with a specified Holder exponent and fundamental
    harmonic :math:`\lambda_0`.

    Details
    -------
    This implementation constructs a finite-sum approximation to the random
    phase Weierstrass function :math:`W_H(t)`, :math:`H\in (0, 1)`, sampled
    over 1d mesh of :math:`n+1` points with spacing :math:`\frac{1}{n}`.
    ...
    random phase shifts of each layer of harmonics. The function is
    approximated by a truncated sum :math:`\sum_{k=-M}^{M} W_k(t)`, where
    :math:`M` governs the fidelity of the approximation, and given by
    ...
    def _onesided(self):
        ...
        lambda_k = 1.0
        for k in range(self.n_layers_ + 1):
        ...
        # No need to subtract w_0 inside the loop, since it cannot
    ...
    def _twosided(self):
        ...
        lambda_k = self.lambda_0
        for k in range(1, self.n_layers_ + 1):
