...
\subsection{$H$-sssi processes} % (fold)
...
The Brownian motion (BM) process $(B(t))_{t\geq0}$ is a zero-mean Gaussian process
...
% subsection h_sssi_processes (end)
...
function, \cite{decrouez2015,decrouez2013estimation}. It exhibits discrete self-similarity:
there exists $a_0 > 0$ such that $f(at) = a^H f(t)$ for all $a\in a_0\mathbb{N}$ and
every $t\geq 0$. This process has continuous, nowhere differentiable paths, and is
defined on $[0,1]$ as the limit of the sum of randomly scaled and shifted cosines
...
The parameter $\lambda_0 > 1$ governs the base scale of the process and is known
as ``the fundamental harmonic''. This process is H\"older continuous with exponent
$H>0$: there exists $C > 0$ such
\begin{equation} \label{eq:def_holder}
    \bigl| W_H(s) - W_H(t) \bigr| \leq C |s - t|^H \,,
\end{equation}
that for all $s,t\geq [0,1]$. Indeed, a trigonometric identity for cosines involving
angles $2\pi \lambda_0^k t + \phi_k + \pi \lambda_0^k \Delta$ and $\pi \lambda_0^k \Delta$
implies that for any $\Delta$ we have
\begin{equation*}
    \bigl| W_H(t+\Delta) - W_H(t) \bigr|
        \leq 2 \sum_{k \geq 0} \lambda_0^{-k H} |\sin(\pi \lambda_0^k \Delta)| \,.
\end{equation*}
Then, letting $K$ be the least $k\geq 0$ such that $\lambda_0^k |\Delta| > 1$, then
