...
By (\ref{eq:subcrossing_partition}) subcrossings of a complete crossing are paired,
...
$\pm\delta 2^n$ levels. Define $V_k^n$ as the movement pattern of the $k$-th $n$-th
level excursion: either `+-' for up-down, or `-+' for down-up.
...
\begin{itemize}
    ...
    \item the excursion directions $V_k^n$ are independent of the crossing
...
properties:
\begin{itemize}
    \item the excursion patterns $V_k^n$ are identically distributed with
    \begin{equation*}
        \pr_+(\text{`+-'}) = \pr_-(\text{`-+'}) = 2^{-2 H^{-1}} \,,
    \end{equation*}
    where $\pr_+$ and $\pr_-$ are conditional probabilities with respect to the
    orientation of the parent crossing;
    \item $Z_k^n$ are possibly correlated but identically distributed according
    to (\ref{conj:subcross});
    \item $(\delta 2^n)^{-\frac{1}{H}} W_k^n$ are identically distributed with mean $1$.
\end{itemize}
With this conjecture it could be possible to use the crossing tree to estimate $H$
based on these properties implied by the conjecture: $H^{-1} \approx \log_2 \ex Z^n$,
or $H^{-1} \approx \log_2 \frac{\ex W^{n+1}}{\ex W^n}$. 

Intuitively, if we one scales the time of an $H$-sssi process by $\ex Z^n$, then
it is necessary to scale down its range on average by $\frac{1}{2}$ -- the resolution
multiplier.
Now, if the process is scale-invariant then the distribution of crossing sizes does
not depend on the level of the tree, and thus it is possible to pool the sample data
of $Z^n_k$ across several levels, over which the hypothesis of common distribution
cannot be rejected.
...
(\ref{eq:def_hermite}), \cite{embrechts2000introduction}.

Sample paths of the Weierstrass function are approximated by truncating (\ref{eq:def_weir})
...
\begin{enumerate}
    \item Take $h$ bi ns $\{2m\}$ for $m = 1,h-1$ and $\{2h, 2h+2,\ldots\}$ and compute
