...
\subsection{Browninan Motion} % (fold)
\label{sub:browninan_motion}
...
% subsection browninan_motion (end)
...
To test the properties of the crossing tree for estimating

we consider the random Weierstrass cascade, \cite{decrouez2013estimation}, which is the limiting random function of
the sum of randomly scaled and shifted cosines
\begin{equation}\label{eq:def_weir}
    f_H(t) = \sum_{k\in \mathbb{Z}} \lambda_0^{-k H} \bigl\{
            \cos \phi_k - \cos(2\pi \lambda_0^k t + \phi_k)
        \bigr\} \,,
\end{equation}
where the random phase shifts $(\phi_k)_{k\in\mathbb{Z}}$ are iid $\mathcal{U}(0, 2\pi)$.

As another example of a non-Gaussian $H$-self-similar ew consider the

The parameter $\lambda_0$ governs the base scale of the process and is known as
``the fundamental harmonic''. The function $f_H(t)$ has continuous paths, but is
nowhere differentiable, even in the deterministic case of $\phi_k=0$.
