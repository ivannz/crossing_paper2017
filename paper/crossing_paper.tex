...
\emph{$n$-th level crossing time}, are defined iteratively: for any $k\geq 0$
...
if this were otherwise, $X(T_m^{n+1}) = X(T_k^n)$, which with the path continuity
...
This key observation is that within any $(n+1)$-st level crossing there must be an
even number of the \emph{$n$-th level subcrossings} -- crossings of the adjacent
finer $n$-th lattice. Indeed, there are two possibilities whenever $T_k^n = T_m^{n+1}$
and $T_{m+1}^{n+1}<+\infty$:
\begin{enumerate}
...
subcrossings (fig.~\ref{fig:xing_tree_structure}), while in the latter -- there are
at least \emph{four}: along with (\ref{eq:duration_partition}) the ordering of crossing
times entails
...
there is an even number of $n$-th level crossings during an $(n+1)$-st level crossing.
...
\begin{figure}
    ...
    %% The anchor and labels
            ...
            \node [draw=none] [right = 0em of uu.east, anchor = west] {$X(T_k^n) + \delta 2^{n+1}$};
            \node [draw=none] [right = 0em of dd.east, anchor = west] {$X(T_k^n) - \delta 2^{n+1}$};
