...
\end{equation}
where $H = k(H_0-1)+1$, and $\int_{\Real^m}'$ is an $m$-dimensional integral over
...
\begin{equation*}
    \biggl(\frac{1}{n^H} \sum_{j=1}^{\lfloor n t\rfloor}
        H_m(\xi_j)\biggr)_{t\geq0} 
            \overset{\Dcal}{\rightarrow} \bigl(Z_H^m(t)\bigr)_{t\geq0} \,,
\end{equation*}
...
from the fractional Gaussian Noise with Hurst index $H_0 = \frac{H-1}{m}+1$,
\cite{embrechts2000introduction}.
